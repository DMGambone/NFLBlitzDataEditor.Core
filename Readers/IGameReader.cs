...
namespace NFLBlitzDataEditor.Core.Readers
{
	public interface IGameReader
	{
		...
		/// <summary>
		/// Returns the font information referenced by the memory address
		/// </summary>
		/// <param name="address">The memory address to read the data from</param>
		/// <returns>An instance of <see cref="FontTable" />.</returns>
		FontTable GetFont(uint address);

		/// <summary>
		/// Returns the list of all the fonts used in the game
		/// </summary>
		/// <returns>A collection of <see cref="FontTable" /> of all fonts defined in the game.</returns>
		IEnumerable<FontTable> GetAllFonts();
	}
}